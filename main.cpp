...
int process_command_line(int argc, char **argv, std::string& model, std::string& formula, double &maxtime, double &checktime, int &buildMode, string& resultfile, std::vector<string> &appendix) {
    try {
        ...
        desc.add_options()
                ...
                ("formula,f", po::value<std::string>(&formula)->default_value("test/TrueFormula.xml"), "Path to the formula .xml file.")
                ...
                ("buildMode,b", po::value<int>(&buildMode)->default_value(0), "choose PLT build method")
...
int main (int argc, char *argv[]) {
    ...
    double checktime;
    int buildMode;
    int mode = process_command_line(argc, argv, modelfile, formulafile, maxtime, checktime, buildMode,resultfile, appendix);
    ...
    if(buildMode!=1) {
        ...
        if (mode == 1) {
            ...
            const auto modelCheckingTime = std::chrono::duration_cast<std::chrono::milliseconds>(
                    endSatisfy - startSatisfy).count();
        ...
        hpnmg::Formula formulaForPLTBuilding = hpnmg::ReadFormula{}.readFormula(formulafile);
        const auto startChecker = std::chrono::high_resolution_clock::now();
        auto checker = hpnmg::RegionModelChecker(*hpng, maxtime, buildMode, checktime, formulaForPLTBuilding);
        const auto endChecker = std::chrono::high_resolution_clock::now();
        auto initTime = std::chrono::duration_cast<std::chrono::milliseconds>(endChecker - startChecker).count();

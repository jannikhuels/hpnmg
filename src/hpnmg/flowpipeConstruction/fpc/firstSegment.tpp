...
State_t<Number> bloatBox( const State_t<Number>& in, const Box<Number>& bloatBox ) {
	...
	template<typename Number>
	boost::tuple<bool, State_t<Number>, matrix_t<Number>, vector_t<Number>> ReachRHPn<Number>::computeFirstSegment( const State_t<Number>& _state ) const {
	...
	template<typename Number>
	matrix_t<Number> ReachRHPn<Number>::computeTrafoMatrix( const Location<Number>* _loc ) const {

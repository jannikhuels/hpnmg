...
namespace hpnmg {
    ...
    private:
        ...
        bool nodeSatisfiesProperty(const ParametricLocation loc, const Formula &formula);
        //helpe method to check a continuous level
        bool getContinuousLevel(const ParametricLocation loc, const std::string& placeId, int value);
        //helpe method get the lower bound of a random variable
        double getLowerBoundHelp(std::vector<std::vector<double>> leftBounds, int s);
        //helpe method to check a discrete value
        bool getDiscreteTokens(const ParametricLocation loc, const std::string& placeId, int value);
        //helpe method to check a conjunction formula
        bool getConjunction(const ParametricLocation loc, const Conjunction& conj);
        //helpe method to check a negation formula
        bool getNegation(const ParametricLocation loc, const Negation& formula);

    public:
        //TODO Check to make it a singleton
        //TODO Change public / private accessor
        PropertyBasedPLTBuilder();
        ...
        // @return                  resulting parametric location tree
        void processNode(ParametricLocationTree::Node node, shared_ptr<HybridPetrinet> hybridPetrinet, double maxTime, double atTime, int mode);

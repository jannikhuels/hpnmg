...
namespace hpnmg {
    ...
    void ParametricLocationTree::recursivelySetRegions(ParametricLocationTree::Node &startNode) {
        ...
        if (childNodes.size() > 0) {
            ...
            Region region = STDiagram::createRegionNoEvent(baseRegion, startNode.getParametricLocation().getSourceEvent(), startNode.getParametricLocation().getGeneralIntervalBoundLeft()[0], startNode.getParametricLocation().getGeneralIntervalBoundRight()[0]);
            startNode.setRegion(region);
        }
    }

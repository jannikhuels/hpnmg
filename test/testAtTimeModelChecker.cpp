...
TEST(RegionModelChecker, LisaTest){
    auto formula = Formula(std::make_shared<Negation>((Formula(std::make_shared<ContinuousAtomicProperty>("pc1", 8)))));
    auto modelChecker = RegionModelChecker(*ReadHybridPetrinet{}.readHybridPetrinet("HA2_Uniform.xml"), 20);
    auto result = modelChecker.satisfies(formula, 10);
    cout << "prob for x>=8 at time 10: " << result.first << endl;
    cout << "error was: " << result.second << endl;

    formula = Formula(std::make_shared<Conjunction>(Formula(std::make_shared<ContinuousAtomicProperty>("pc1",8)),
            Formula(std::make_shared<Negation>((Formula(std::make_shared<ContinuousAtomicProperty>("pc1", 5)))))));
    result = modelChecker.satisfies(formula, 10);
    cout << "prob for x>=5 and <8 at time 10: " << result.first << endl;
    cout << "error was: " << result.second << endl;
    //nur als alibi
    EXPECT_NEAR(0.8, round(result.first*10)/10, result.second);
}
TEST(RegionModelChecker, LisaTest2){
   auto formula = Formula(std::make_shared<Negation>((Formula(std::make_shared<ContinuousAtomicProperty>("pc1", 8)))));
    auto modelChecker = RegionModelChecker(*ReadHybridPetrinet{}.readHybridPetrinet("HA2_Expo.xml"), 20);
    auto result = modelChecker.satisfies(formula, 10);
    cout << "prob for x>=8 at time 10: " << result.first << endl;
    cout << "error was: " << result.second << endl;
    ...
    cout << "error was: " << result.second << endl;
    //nur als alibi
    EXPECT_NEAR(0.2, round(result.first*10)/10, result.second);
}
...
//test cases with until formula start here

...
TEST(PropertyBasedPLTBuilder, annabellExample){
  auto        reader         = new ReadHybridPetrinet();
    auto        hybridPetrinet = reader->readHybridPetrinet("annabell_example.xml");
    auto        builder        = new ParseHybridPetrinet();                                                                                            
    auto        plt            = builder->parseHybridPetrinet(hybridPetrinet, 5);
    auto        initState      = plt->getRootNode().getParametricLocation();
    auto        writer         = new PLTWriter();
    writer->writePLT(plt, 100, "annabellPLTParser");
    std::vector<ParametricLocationTree::Node> nodes = plt->getAllLocations();
    for(int i=0; i< nodes.size(); i++){
       cout<<"Node ID: "<< nodes[i].getNodeID()<<endl;
       std::vector<std::vector<double>> cm = nodes[i].getParametricLocation().getContinuousMarking();
       std::vector<std::vector<std::vector<double>>> gBL = nodes[i].getParametricLocation().getGeneralIntervalBoundLeft();
       for(int j=0; j<cm.size(); j++){
           cout<<"fluid level at place: "<<j<<endl;
           for(int k=0; k<cm.at(0).size(); k++){
              cout<<"fluid level: "<< cm[j][k]<<endl;
           }
       }
       for(int l=0; l<gBL.size(); l++){
           for(int m=0; m<gBL.at(l).size(); m++){
               for(int n=0; n< gBL.at(l).at(m).size(); n++){
                   cout<<"left bound: "<< gBL[l][m][n]<< "at: "<<l<<m<<n<<endl;
               }
    ...
    ASSERT_EQ(8, plt->numberOfLocations());
    ASSERT_EQ(1, plt->getChildNodes(plt->getRootNode()).size());
    auto        children       = plt->getChildNodes(plt->getRootNode());
    ASSERT_EQ(2, plt->getChildNodes(children[0]).size());
...
TEST(PropertyBasedPLTBuilder, ExampleTrueUntilb){
    ...
    auto	plt	       = builder->parseHybridPetrinet(hybridPetrinet, 10, 2, 0, formula);
    ASSERT_EQ(6, plt->numberOfLocations());
    ...
    ASSERT_EQ(9, plt->numberOfLocations());
...
TEST(PropertyBasedPLTBuilder, ExampleaUntilb) {
    ...
    auto writer = new PLTWriter();
    auto formula = Formula(std::make_shared<Until>(
            Formula(std::make_shared<DiscreteAtomicProperty>("pd1", 1)), Formula(std::make_shared<DiscreteAtomicProperty>("pd2", 0)), 10));

    auto plt = builder->parseHybridPetrinet(hybridPetrinet, 10, 2, 0, formula);
    ASSERT_EQ(6, plt->numberOfLocations());

    formula = Formula(std::make_shared<Until>(
            Formula(std::make_shared<DiscreteAtomicProperty>("pd1", 0)), Formula(std::make_shared<DiscreteAtomicProperty>("pd2", 1)), 10));

    plt = builder->parseHybridPetrinet(hybridPetrinet, 10, 2, 0, formula);
    writer -> writePLT(plt, 10);
    ASSERT_EQ(5, plt->numberOfLocations());
}


TEST(PropertyBasedPLTBuilder, LisaExampleWithUntil){
    auto reader = new ReadHybridPetrinet();
    ...
    auto formula = Formula(std::make_shared<Until>(
        Formula(std::make_shared<True>()), Formula(std::make_shared<Conjunction>(Formula(std::make_shared<ContinuousAtomicProperty>("pc1", 9)),
            Formula(std::make_shared<ContinuousAtomicProperty>("pc2", 4)))), 5));

    auto builder = new  PropertyBasedPLTBuilder();

    const auto	startChecker = std::chrono::high_resolution_clock::now();
    auto plt = builder->parseHybridPetrinet(hybridPetrinet, 5, 2, 0, formula);
    const auto	endChecker   = std::chrono::high_resolution_clock::now();
    auto	initTime     = std::chrono::duration_cast<std::chrono::milliseconds>(endChecker - startChecker).count();
    cout << "[Parsing with PropertyBasedPLTBuilder]: " << initTime << "ms" << endl;
    auto writer = new PLTWriter();
    writer -> writePLT(plt, 10, "lisa_result");
    ASSERT_EQ(11, plt->numberOfLocations());

    //compare to PLT generated by ParseHybridPetrinet
    auto parser = new ParseHybridPetrinet();
    const auto	startChecker2 = std::chrono::high_resolution_clock::now();
    plt = parser->parseHybridPetrinet(hybridPetrinet, 5);
    const auto	endChecker2   = std::chrono::high_resolution_clock::now();
    ...
    ASSERT_EQ(21, plt->numberOfLocations());
}


TEST(PropertyBasedPLTBuilder, BuildPLTForECharging){
    auto reader = new ReadHybridPetrinet();
    auto hybridPetrinet = reader->readHybridPetrinet("ev_charging.xml");
    auto parser = new PropertyBasedPLTBuilder();
    auto plt = parser->parseHybridPetrinet(hybridPetrinet, 10, 4);
    auto writer = new PLTWriter();
    writer->writePLT(plt, 10, "Echarging_result.xml");
    //auto initState = plt->getRootNode().getParametricLocation();
    ASSERT_EQ(plt->numberOfLocations(), 11);

    auto formula = Formula(std::make_shared<Until>(
        Formula(std::make_shared<True>()), Formula(std::make_shared<DiscreteAtomicProperty>("unstable_grid", 1)), 10));
    plt = parser->parseHybridPetrinet(hybridPetrinet, 10, 4, 0, formula);
    ASSERT_EQ(plt->numberOfLocations(), 46);

    formula = Formula(std::make_shared<Until>(
        Formula(std::make_shared<True>()), Formula(std::make_shared<DiscreteAtomicProperty>("driving", 1)), 10));
    plt = parser->parseHybridPetrinet(hybridPetrinet, 10, 2, 0, formula);
    ASSERT_EQ(plt->numberOfLocations(), 130);

}


//the following tests were also executed for ParseHybridPetrinet
